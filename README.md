...
同一 JSON 对象中**重复出现同名字段**属于含歧义的请求，同样返回 422 并定位到重复
字段，而不是静默采用后一个值：根对象重复 `measurements` 数组定位到 `measurements`；
测点内重复 `id` / `flow_lph` / `rated_flow_lph` 定位到 `measurements[i].<字段>`，
即使后一个值合法也不例外（稳定性接口的 `rounds`、轮次内字段同理）。
...
重复 `measurements` 数组与测点内重复 `flow_lph` / `rated_flow_lph` 的拒绝与定位、
